...
{ 
	// A AUX_TABLE CORRESPONDE Á NEXT TABLE
    int num_live = 0;
...
int num_alive_neighbours_d2(cell * table, cell * aux_table, cell * aux_low_table, cell * aux_high_table, cell* lower_row_table, cell* upper_row_table, int x, int y, int z, cell * lowerB, cell * higherB, int lowB, int highB, int SIZE_TABLE, int SIZE_CUBE, int SIZE_B_TABLE, int * num_down, int * num_up)
{
    ...
    if(!find) {   //ENTRA SE VIR QUE A CELULA ESTA MORTA, QUE AINDA NAO TENHA SIDO ADICIONADA NA PROXIMA GERAÇÃO!
        //THE ONLY CHANGE NECESSARY IS IN THE X COORDINATES. WE CHECK IF THE NEIGHBOURS WE WANT TO SEARCH ARE OUTSIDE THE BOUNDRIES OF
        //table, IF THEY ARE WE SEARCH IN lowerB TABLE (ARRAY WITH ALL THE NODES IN THE LOWER BOUND) OR IN THE TABLE higherB (HIGHER BOUNDRY)
        
        //VERIFICAÇÃO SE OS VIZINHOS DA CELULA MORTA ESTAO VIVOS PARA ESTA PODER OU NAO FICAR VIVA NA PROXIMA GERAÇÃO
        
        if(mod((x+1),SIZE_CUBE)==highB) num_live += search(higherB, mod((x+1),SIZE_CUBE), y, z, SIZE_B_TABLE); //SE X+1 ESTA NA GHOST UP ENTAO FAZ UM SEARCH DA CELULA NA HIGH MAIN TABLE 
        else num_live += search(table, mod((x+1),SIZE_CUBE), y, z, SIZE_TABLE); // SENAO FAZ SEARCH NA MAIN TABLE
        ...
        if(num_live==2 || num_live==3){
            ...
            insert(aux_table, x, y, z, SIZE_TABLE); //SE O NUMERO DE VIZINHOS VIVOS DA CELULA MORTA ESTIVER  DENTRO DESTE RANGE, ENTAO ADICIONA ESTA CELULA MORTA COMO VIVA NA PROXIMA GERAÇÃO ADICIONANDO A NA AUX TABLE!

            if (x == mod(lowB+1, SIZE_CUBE) || x == mod(lowB+2, SIZE_CUBE)){ //SE A CELULA QUE VAI ESTAR NA PROXIMA GERAÇÃO PERTENCER ?????????????????
            ...
            if (x == mod(lowB+1, SIZE_CUBE)){ //ATUALIZAR A BOUND LOW
                insert(lower_row_table, x, y, z, SIZE_B_TABLE);
            }

            if (x == mod(highB-1, SIZE_CUBE)){ // ATUALIZAR A BOUND HIGH
...
int num_alive_neighbours_a2(cell * table, cell * aux_table, cell * aux_low_table, cell * aux_high_table, cell* lower_row_table, cell* upper_row_table, int x, int y, int z, cell * lowerB, cell * higherB, int lowB, int highB, int SIZE_TABLE, int SIZE_CUBE, int SIZE_B_TABLE, int * num_down, int * num_up)
{
    ...
    //THE ONLY CHANGE NECESSARY IS IN THE X COORDINATES7
    //MESMO PROCEDIMENTO: VERIFICA SE X +/- 1 NAO PERTENCE A UMA DAS GHOST ROWS. SE O VIZINHO NAO PERTENCER VAI VER SE ELE ESTA VIVO COM A FUNCAR SEARCH!
    //SE NAO ESTIVER VIVO VAI-SE CONTAR O NUMERO DE VIZINHOS VIVOS DESSA CELULA MORTA PARA VER SE FICA VIVO NA PROXIMA GERAÇÃO!
    if(mod((x+1),SIZE_CUBE)!=highB){
        if(!search(table, mod((x+1),SIZE_CUBE), y, z, SIZE_TABLE)) 
...
{ 
    ...
    if(!find) {    
        //THE ONLY CHANGE NECESSARY IS IN THE X COORDINATES. WE CHECK IF THE NEIGHBOURS WE WANT TO SEARCH ARE OUTSIDE THE BOUNDRIES OF 
        //table, IF THEY ARE WE SEARCH IN lowerB TABLE (ARRAY WITH ALL THE NODES IN THE LOWER BOUND) OR IN THE TABLE higherB (HIGHER BOUNDRY) 
        if(mod((x+1),SIZE_CUBE)==highB) num_live += search(higherB, mod((x+1),SIZE_CUBE), y, z, SIZE_B_TABLE); // SE O VIZINHO DA CELULA MORTA ESTA NUMA GHOST ROW
        else num_live += search(table, mod((x+1),SIZE_CUBE), y, z, SIZE_TABLE); // SE O VIZNHO DA CELULA MORTA ESTA NA MAIN TABLE, OU SEJA, NAO ESTA NA HIGH BOUND DESSE PROCESSO. 
...
{ 
    ...
    //THE ONLY CHANGE NECESSARY IS IN THE X COORDINATES 
    if(mod((x+1),SIZE_CUBE)!=highB){ //SE O X+1 NAO CORRESPONDER AO GHOST DOWN, ENTAO FAZER O PROCEDIMENTO NORMAL.
        if(!search(table, mod((x+1),SIZE_CUBE), y, z, SIZE_TABLE))//SE NAO ENCONTRAR A CELULA VIVA, ENTAO ELA ESTA MORTA:  
            num_alive_neighbours_df(table, aux_table, mod((x+1),SIZE_CUBE), y, z, lowerB, higherB, lowB, highB, SIZE_TABLE, SIZE_B_TABLE, SIZE_CUBE);//VAI PROCURAR CELULAS VIVAS Á VOLTA DA CELULA MORTA, PARA VER SE FICA VIVA NA PROXIMA ITERAÇÃO
        else num_live++; 
    }else{ 
        if(search(higherB, mod((x+1),SIZE_CUBE), y, z, SIZE_B_TABLE))  num_live++; //SE ENCONTRAR A CELULA VIVA HIGH MAIN TABLE
    } 
